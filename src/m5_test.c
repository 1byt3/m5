...
int test_m5_add_u16(void)
{
	struct app_buf buf = { .data = buffer, .len = 0, .offset = 0,
			       .size = sizeof(buffer)};
...
int test_m5_add_u32(void)
{
	struct app_buf buf = { .data = buffer, .len = 0,
			       .size = sizeof(buffer)};

	m5_add_u32(&buf, 0xABCDEF01);
	if (buf.data[0] != 0xAB || buf.data[1] != 0xCD ||
	    buf.data[2] != 0xEF || buf.data[3] != 0x01) {
		return -EINVAL;
	}
...
int test_m5_add_str(void)
{
	struct app_buf buf = { .data = buffer, .len = 0, .offset = 0,
			       .size = sizeof(buffer)};
	const char *str = "mqtt5 add str test";
...
void print_prop(struct m5_prop *p)
{
	uint32_t i;
...
int compare_topics(struct m5_topics *t1, struct m5_topics *t2)
{
	uint8_t i;
	int rc;
...
int main(void)
{
	...
	printf("Prop: %zu\n", sizeof(struct m5_prop));

	rc = test_m5_add_u16();
	printf("[%s] test_m5_add_u16\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_add_u32();
	printf("[%s] test_m5_add_u32\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_add_str();
	printf("[%s] test_m5_add_str\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_connect();
	printf("[%s] test_m5_connect\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_connack();
	printf("[%s] test_m5_connack\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_publish();
	printf("[%s] test_m5_publish\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_subscribe();
	printf("[%s] test_m5_subscribe\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_suback();
	printf("[%s] test_m5_suback\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_unsubscribe();
	printf("[%s] test_m5_unsubscribe\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_unsuback();
	printf("[%s] test_m5_unsuback\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_pings();
	printf("[%s] test_m5_pings\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_disconnect();
	printf("[%s] test_m5_disconnect\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}

	rc = test_m5_auth();
	printf("[%s] test_m5_auth\n", RC_TO_STR(rc));
	if (rc != EXIT_SUCCESS) {
		goto lb_exit;
	}
